...
def add_battery_constraints(n, planning_horizons=None):
    ...
    investment_year = int(planning_horizons) if planning_horizons is not None else None
...
def add_iron_air_constraints(n):
    ...
    """
    options = n.config.get("electricity", {}).get("storage_options", {}).get("iron-air")
    if not options:
        return
    ...
    chargers = n.links.query("carrier == 'iron-air charger' and p_nom_extendable")
    dischargers = n.links.query("carrier == 'iron-air discharger' and p_nom_extendable")
    ...
    dischargers_by_bus = component_by_internal_bus(dischargers, "bus0", "discharger")
    ...
    if not (
        ...
        raise ValueError("Iron-air Store, charger, and discharger buses do not match.")

    for position, internal_bus in enumerate(sorted(store_buses)):
