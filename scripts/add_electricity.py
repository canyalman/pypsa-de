...
def attach_stores(
    ...
    for carrier in available_carriers:
        ...
        if carrier == "iron-air":
            ...
            # Charge the complete fixed-100-hour system cost once to the
            ...
            charge_capital_cost = (
                (calculate_annuity(lifetime, discount_rate) + fom) * investment * nyears
            )
...
def attach_continuous_acaes(
    ...
    if missing_costs:
        raise ValueError(
            "Missing continuous RESC A-CAES costs: " + ", ".join(sorted(missing_costs))
        )
    ...
    output_vom = float(costs["vom_usd2022_per_mwh_output"]) * configured_factor
    ...
    n.carriers.loc[[carrier, charger_carrier, discharger_carrier], "co2_emissions"] = (
        0.0
    )
    ...
    n.add(
        ...
        capital_cost=output_energy_cost * 1e3 * efficiency_dispatch * nyears,
        marginal_cost=0.0,
        lifetime=lifetime,
    ...
    n.stores.loc[internal_buses, "acaes_output_energy_factor"] = efficiency_dispatch
