...
def test_production_config_uses_approved_continuous_formulation():
    ...
    assert config["run"]["prefix"] == "storage_competition_free_eu_1h"
    assert config["run"]["name"] == ["KN2045_Mix_FixedRenewables"]
    assert config["clustering"]["temporal"]["resolution_sector"] == "1H"
    assert "fixed_neighbor_capacities" not in config["solving"]["constraints"]
    assert config["common_reference_history"]["reuse_reference_runs"] == [
...
def test_attachment_maps_output_costs_and_geology_to_internal_units():
    ...
    eta = np.sqrt(options["round_trip_efficiency"])
...
def test_continuous_acaes_delivers_eight_hours_at_69_percent_rte():
    ...
    assert np.isclose(
        discharge_output / charge_input, options["round_trip_efficiency"]
    )
