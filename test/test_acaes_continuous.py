...
def _constraint_function():
    module_path = REPO_ROOT / "scripts" / "pypsa-de" / "additional_functionality.py"
...
def test_production_config_uses_approved_continuous_formulation():
    ...
    assert config["run"]["prefix"] == "storage_competition_fixed_nonde_3h_2025base"
    assert config["run"]["name"] == ["KN2045_Mix_FixedRenewables"]
    assert config["electricity"]["extendable_carriers"]["Store"] == [
        "battery",
        "H2",
        "iron-air",
    ]
